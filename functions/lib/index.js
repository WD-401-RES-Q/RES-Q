...
function hashRecoveryOtpCode(pepper, challengeId, otpCode) {
    return hashSha256(`${pepper}:${challengeId}:${otpCode}`);
}
function hashRecoverySessionToken(pepper, sessionToken) {
    return hashSha256(`${pepper}:session:${sessionToken}`);
}
function generateRecoveryOtpCode() {
    return (100000 + crypto.randomInt(900000)).toString();
}
function readRecoveryOtpPepper() {
    var _a;
    const pepper = readTrimmedString((_a = functions.config().recovery) === null || _a === void 0 ? void 0 : _a.otp_pepper);
    if (pepper.length < 16) {
        throw new functions.https.HttpsError('failed-precondition', 'Server recovery OTP configuration is missing.');
    }
    return pepper;
}
...
function buildRecoveryOtpEmailPayload(otpCode) {
    const subject = 'RES-Q Account Recovery Code';
    const text = `Your RES-Q account recovery code is ${otpCode}. ` +
...
async function enqueueRecoveryOtpEmail(email, otpCode) {
...
async function findAccountByEmailInCollection(collectionName, normalizedEmail, rawEmail) {
...
async function findRecoveryAccountByEmail(rawEmail) {
    const normalizedEmail = normalizeRecoveryEmail(rawEmail);
    for (const collectionName of RECOVERY_ACCOUNT_COLLECTIONS) {
        const doc = await findAccountByEmailInCollection(collectionName, normalizedEmail, rawEmail);
        if (doc != null) {
            return {
                collection: collectionName,
                doc,
...
async function hasRecoveryPhoneConflict(phoneNumber, excludedAccountKeys) {
 ...
 */
exports.requestAccountRecoveryOtp = functions
    .region('asia-east2')
    .https.onCall(async (data, context) => {
    var _a;
    try {
        ...
        let existingChallengeData = null;
        if (requestedChallengeId.length > 0) {
            ...
            if (existingSnapshot.exists) {
                const existingData = ((_a = existingSnapshot.data()) !== null && _a !== void 0 ? _a : {});
                const sameEmail = readTrimmedString(existingData.emailLower) === normalizedEmail;
                const sameAccount = readTrimmedString(existingData.accountCollection) ===
        ...
        if (existingChallengeData) {
            ...
            if (lastSentAtMs != null &&
                ...
                const remainingMs = RECOVERY_RESEND_COOLDOWN_MS - (nowMs - lastSentAtMs);
        ...
        const writePayload = {
            ...
            otpHash,
            verifyAttempts: 0,
            expiresAt: admin.firestore.Timestamp.fromMillis(nowMs + RECOVERY_OTP_TTL_MS),
        ...
        throw new functions.https.HttpsError('internal', 'Failed to send account recovery code.');
    }
});
/**
 * Verifies a recovery OTP and returns a short-lived recovery session token.
 */
exports.verifyAccountRecoveryOtp = functions
    .region('asia-east2')
    .https.onCall(async (data, context) => {
    var _a;
    try {
        ...
        if (!/^[0-9]{6}$/.test(otpCode)) {
            throw new functions.https.HttpsError('invalid-argument', 'OTP must be a 6-digit code.');
        }
        ...
        if (!challengeSnapshot.exists) {
            throw new functions.https.HttpsError('not-found', 'Recovery session not found. Please request a new code.');
        }
        const challengeData = ((_a = challengeSnapshot.data()) !== null && _a !== void 0 ? _a : {});
        if (toTimestampMillis(challengeData.consumedAt) != null) {
            throw new functions.https.HttpsError('failed-precondition', 'Recovery session has already been used.');
        }
        ...
        const verifyAttempts = typeof challengeData.verifyAttempts === 'number'
        ...
        await challengeRef.set({
            ...
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            sessionTokenHash,
            sessionExpiresAt: admin.firestore.Timestamp.fromMillis(Date.now() + RECOVERY_SESSION_TTL_MS),
        ...
        throw new functions.https.HttpsError('internal', 'Failed to verify recovery code.');
    }
});
/**
 * Completes account recovery by updating phone and PIN after OTP verification.
 */
exports.completeAccountRecovery = functions
    .region('asia-east2')
    .https.onCall(async (data, context) => {
    var _a, _b;
    try {
        ...
        if (normalizedPhone == null) {
            throw new functions.https.HttpsError('invalid-argument', 'A valid Philippine mobile number is required.');
        }
        if (!/^[0-9]{4}$/.test(pin)) {
            throw new functions.https.HttpsError('invalid-argument', 'PIN must be a 4-digit code.');
        }
        const tokenSeparatorIndex = sessionToken.indexOf('.');
        if (tokenSeparatorIndex <= 0 ||
            tokenSeparatorIndex >= sessionToken.length - 1) {
            throw new functions.https.HttpsError('invalid-argument', 'Invalid recovery session token.');
        }
        ...
        const challengeSnapshot = await challengeRef.get();
        if (!challengeSnapshot.exists) {
            throw new functions.https.HttpsError('not-found', 'Recovery session not found. Please restart recovery.');
        }
        const challengeData = ((_a = challengeSnapshot.data()) !== null && _a !== void 0 ? _a : {});
        if (toTimestampMillis(challengeData.consumedAt) != null) {
            throw new functions.https.HttpsError('failed-precondition', 'Recovery session has already been used.');
        ...
        if (expectedSessionHash.length === 0) {
            throw new functions.https.HttpsError('failed-precondition', 'Recovery session is not verified.');
        }
        const incomingSessionHash = hashRecoverySessionToken(otpPepper, sessionToken);
        if (incomingSessionHash !== expectedSessionHash) {
            throw new functions.https.HttpsError('permission-denied', 'Invalid recovery session. Please verify your code again.');
        }
        ...
        if (sessionExpiresAtMs == null || Date.now() > sessionExpiresAtMs) {
            throw new functions.https.HttpsError('deadline-exceeded', 'Recovery session expired. Please verify your code again.');
        }
        ...
        if (!RECOVERY_ACCOUNT_COLLECTIONS.includes(accountCollection) ||
            accountDocId.length === 0) {
            throw new functions.https.HttpsError('failed-precondition', 'Recovery target account is invalid.');
        }
        const accountRef = db.collection(accountCollection).doc(accountDocId);
        const accountSnapshot = await accountRef.get();
        if (!accountSnapshot.exists) {
            throw new functions.https.HttpsError('not-found', 'Account record not found.');
        }
        const accountData = ((_b = accountSnapshot.data()) !== null && _b !== void 0 ? _b : {});
        const updateTargets = new Map();
        const addUpdateTarget = (collectionName, docId) => {
            if (!RECOVERY_ACCOUNT_COLLECTIONS.includes(collectionName) ||
                docId.trim().length === 0) {
                return;
            }
        ...
        const previousPhoneCandidates = new Set();
        const normalizedContactNumber = normalizePhilippineMobile(accountData.contactNumber);
        ...
        const excludedAccountKeys = new Set(updateTargets.keys());
        const hasConflict = await hasRecoveryPhoneConflict(normalizedPhone, excludedAccountKeys);
        if (hasConflict) {
            throw new functions.https.HttpsError('already-exists', 'Phone number already in use. Please try another one.');
        }
        const hashedPin = hashSha256(pin);
        const baseAccountUpdates = {
        ...
        for (const target of updateTargets.values()) {
            const accountUpdates = Object.assign({}, baseAccountUpdates);
        ...
        throw new functions.https.HttpsError('internal', 'Failed to complete account recovery.');
    }
});
/**
